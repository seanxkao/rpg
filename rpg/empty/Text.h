...
public:
	...
	Text(){
		...
		D3DXCreateFont(drawer->getDevice() , 46, 0, FW_NORMAL, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Arial", &font);   
	}
	Text(wstring word, int colorA, int colorR, int colorG, int colorB){
		extern Drawer *drawer;
		exist = false;
		setFixed(false);
		setWord(word);
...
public:
	...
	void addText(wstring word, float left, float top, float width, float height, int colorA, int colorR, int colorG, int colorB, int inTime,int inMode,float inX,float inY, int onTime,int onMode,float onX,float onY, int outTime,int outMode,float outX,float outY){
		for(int i = 0; i < size; i++){
			if(!text[i].isExist()){
